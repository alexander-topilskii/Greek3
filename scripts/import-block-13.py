#!/usr/bin/env python3
"""Import textbook block 13 vocabulary from lexicon into words/ + blocks/13/readme.md."""
...
LEXICON = Path("/home/ubuntu/.cursor/projects/workspace/uploads/lexicon_d854.md")
...
def classify(greek: str, russian: str) -> tuple[str, str]:
    ...
    g = greek.strip()
    gl = g.lower()
    # phrases: multiword, punctuation, questions, exclamations, particles with spaces
    ...
    if has_punct or (multi and not gl.startswith(("το ", "τα ", "η ", "ο ", "οι ", "τον ", "την ", "του ", "της "))):
        # still could be noun with article
        if gl.startswith(("το ", "τα ", "η ", "ο ", "οι ")) and len(tokens) <= 2 and not has_punct:
            return "nouns", "Блок 13"
        return "phrases", "Блок 13"
    # verbs: often end with ω/ομαι/μαι or have · aorist
    if " · " in g:
        first = g.split(" · ")[0].strip().lower()
        if first.endswith(("ω", "ώ", "ομαι", "ιέμαι", "άμαι", "ούμαι")) or first in {"είμαι", "έχω", "λέω", "τρώω", "πάω", "ζω"}:
            return "verbs", "Блок 13"
    lemma = tokens[0] if tokens else gl
    if lemma.endswith(("ω", "ώ", "ομαι", "ιέμαι", "άμαι", "ούμαι")) and len(tokens) == 1:
        return "verbs", "Блок 13"
...
SECTION_TOPICS = {
    "Поздравления и пожелания": ["Праздники"],
    "День рождения и именины": ["Праздники"],
    "Вопросы о жизни": ["Биография"],
    "Биография: ключевые глаголы (аорист)": ["Биография"],
    "Аорист: τύπος Α (основные)": ["Базовые глаголы"],
    "Аорист: неправильные и особые формы": ["Базовые глаголы"],
    "Время и прошлое": ["Время"],
    "Семья и люди": ["Семья"],
    "Работа, учёба, профессии": ["Работа"],
    "Путешествие и миграция": ["Путешествия"],
    "Здоровье и происшествия": ["Здоровье"],
    "Дом и быт": ["Дом"],
    "Школа и класс": ["Учёба"],
    "Движение и повседневные действия": ["Повседневность"],
    "Разговорные фразы": ["Разговорные"],
    "Профессии: что делал вчера": ["Работа"],
    "Поэзия и литература": ["Литература"],
    "Прочее из урока": ["Общее"],
    "Тема урока": ["Биография"],
}
...
def create_or_resolve(section: str, greek: str, russian: str) -> tuple[str | None, str]:
    ...
    topics = SECTION_TOPICS.get(section, ["Блок 13"])
