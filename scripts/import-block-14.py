#!/usr/bin/env python3
"""Import textbook block 14 vocabulary from lexicon into words/ + blocks/14/readme.md."""
...
LEXICON = Path("/home/ubuntu/.cursor/projects/workspace/uploads/lexicon_993a.md")
...
def classify(greek: str, russian: str) -> tuple[str, str]:
    ...
    # verbs first (present · aorist · future)
    if " · " in g or " / " in g:
    ...
    if has_punct or (multi and not gl.startswith(("το ", "τα ", "η ", "ο ", "οι ", "τον ", "την ", "του ", "της "))):
        if gl.startswith(("το ", "τα ", "η ", "ο ", "οι ")) and len(tokens) <= 2 and not has_punct:
            return "nouns", "Блок 14"
        return "phrases", "Блок 14"
...
SECTION_TOPICS = {
    "Будущее время и грамматика": ["Грамматика"],
    "Время и планы": ["Время"],
    "Разговорные фразы и диалог": ["Разговорные"],
    "В таверне: заказ и еда": ["Еда и напитки"],
    "Глаголы: настоящее · аорист · будущее": ["Базовые глаголы"],
    "Глаголы типа B1–B2": ["Базовые глаголы"],
    "Планы, учёба и путешествия": ["Путешествия"],
    "Первые разы и жизненные ситуации": ["Биография"],
    "Погода, место и быт": ["Погода"],
    "Полезные слова и выражения урока": ["Общее"],
}
...
def create_or_resolve(section: str, greek: str, russian: str) -> tuple[str | None, str]:
    ...
    topics = SECTION_TOPICS.get(section, ["Блок 14"])
