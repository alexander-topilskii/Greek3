...
import { outputDirFor, renderHome, renderIndex, renderWord, sitePath, wordOutputPath } from './render';
...
function breadcrumbsForWord(entry: WordEntry): { label: string; href?: string }[] {
  const crumbs: { label: string; href?: string }[] = [
    { label: 'Главная', href: sitePath('index.html') },
  ];

  if (entry.category) {
    crumbs.push({ label: entry.category, href: sitePath(`words/${entry.category}/index.html`) });
  }
...
function breadcrumbsForIndex(relativePath: string, title: string): { label: string; href?: string }[] {
  const crumbs: { label: string; href?: string }[] = [
    { label: 'Главная', href: sitePath('index.html') },
  ];
...
function main(): void {
  ...
  for (const file of mdFiles) {
    ...
    if (relative.toLowerCase().endsWith('readme.md')) {
      ...
      const html = renderIndex(index, pageDir, breadcrumbsForIndex(relative, index.title));
      writeHtml(out, html);
      console.log(`  📄 ${out}`);
      continue;
    }
