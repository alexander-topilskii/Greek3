...
export function resolveWordMeta(
  ...
  return {
    level: word.meta.level || CATEGORY_DEFAULT_LEVEL[word.category] || 'A2',
    topics: word.meta.topics.length ? word.meta.topics : inferredTopics,
    tags: word.meta.tags,
...
export function enrichWordEntry(word: WordEntry, inferredTopics: string[] = []): WordEntry {
  const meta = resolveWordMeta(word, inferredTopics);
  return { ...word, meta, primaryGreek: primaryGreekForm(word) };
}

function topicTitleFromSlug(slug: string, words: CatalogWord[]): string {
