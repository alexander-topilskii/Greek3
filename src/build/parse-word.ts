...
function emptyMeta(): WordMeta {
  return { level: '', topics: [], tags: [], recordType: '' };
}
...
export function parseWordFile(filePath: string, wordsRoot: string): WordEntry {
  ...
  for (const section of sections) {
    ...
    } else if (!SPECIAL_SECTIONS.has(key)) {
...
export function getSpecialSection(word: WordEntry, title: string): { title: string; lines: string[] } | null {
